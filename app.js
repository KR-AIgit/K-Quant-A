...
const STOCK_DATABASE = {
    "005930": {
        ...
        aiComments: [
            { author: "K-Stock Agent", sentiment: "bullish", text: "하방 경직성이 확보된 상태에서 외인 수급이 연속으로 들어오고 있어 단기 8만 전자 돌파 시도가 매우 유력합니다.", time: "10분 전" },
            { author: "Value Quant Bot", sentiment: "bullish", text: "현재 PBR은 과거 10년 밴드 최하단에 위치합니다. 배당수익률을 고려할 때 현 구간은 매우 안전한 장기 매집 구간입니다.", time: "2시간 전" }
        ],
        volatility: 0.008,
        basePrice: 78000
    },
    "000660": {
        ...
        aiComments: [
            { author: "HBM Tracker", sentiment: "bullish", text: "글로벌 AI 반도체 체인에서 SK하이닉스의 독점적 지위는 당분간 깨지기 힘듭니다. 조정 시 적극 매수 관점 유지.", time: "1시간 전" }
        ],
        volatility: 0.015,
        basePrice: 195000
    },
    "069500": {
        ...
        aiComments: [
            { author: "Passive Investor AI", sentiment: "bullish", text: "정부 밸류업 모멘텀으로 저PBR 가치주들이 재평가 받으면서 코스피 200 전체의 멀티플 상향이 이루어지고 있습니다.", time: "30분 전" }
        ],
        volatility: 0.005,
        basePrice: 38200
    },
    "114800": {
        ...
        aiComments: [
            { author: "Hedge Master", sentiment: "neutral", text: "코스피가 전고점 저항에 부딪힌 상태입니다. 현 구간에서 비중 10~20% 정도는 인버스로 단기 헷지를 걸어두는 것이 AI 손익비 모델상 합리적입니다.", time: "15분 전" }
        ],
        volatility: 0.005,
        basePrice: 4200
    }
...
function getStockInfo(code) {
    ...
    const newStock = {
        ...
        aiComments: [
            { author: "System Bot", sentiment: "neutral", text: "실시간 연동 모니터링이 시작되었습니다.", time: "1분 전" }
        ],
        volatility: 0.015,
...
// Chart filters removed
    ...
    if (searchBtn && searchInput) {
        searchBtn.addEventListener("click", () => {
            ...
            const select = document.getElementById("stock-select");
            let exists = Array.from(select.options).some(opt => opt.value === code);
            
            if (!exists) {
                const searchedOptgroup = document.getElementById("searched-optgroup");
                searchedOptgroup.style.display = "block";
                const newOpt = document.createElement("option");
                newOpt.value = code;
                
                let optName = "검색된 종목 (" + code + ")";
                for(let key in STOCK_NAME_MAP) {
                    if(STOCK_NAME_MAP[key] === code) { optName = key + " (" + code + ")"; break; }
                }
                newOpt.textContent = optName;
                searchedOptgroup.appendChild(newOpt);
            }
